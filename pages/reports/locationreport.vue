        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Assets In Locations</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <div class="content">
      <div class="container-fluid">
        <b-row
          ...
          class="m-0 p-0"
        >
          <b-col lg="3" class="mx-0 px-0 pr-sm-1 pr-md-1">
          ...
          <b-col lg="3" class="mx-0 px-0 mt-0 mb-0 pr-sm-1 pr-md-1">
            <b-form-group :label="float_label ? null : 'Filter Location(s)'" :label-cols-sm="float_label ? null : 2" class="mb-1">
              <span :class="float_label ? 'has-float-label' : null">
                <div>
                <v-select
                  ...
                  :enabled="!form.processing"
                >
                  <!-- <template #selected-option="{ loc }">
                    <span style="background: transparent;">{{ loc.name }}</span>
                  </template> -->
                </v-select>
                </div>
                <span v-if="float_label">Filter Location(s)</span>
              </span>
            </b-form-group>
          </b-col>
...
export default {
  ...
  async asyncData ({ $axios, route }) {
    let mGroups = []
    let mLocations = []
    await $axios.$get('/api/reports/datas?datas=groups,locations' + (route.query.scope ? '&scope=' + route.query.scope : ''))
      .then((res) => {
        mGroups = res.groups
        mLocations = res.locations
      })
      .catch(() => {})
    return { mGroups, mLocations }
  },
  data () {
    return {
  ...
  methods: {
    async generateReport () {
      ...
      await this.$axios.$get(`/api/reports/locations?groups=${this.form.groups}&locations=${this.form.locations}` + (this.$route.query.scope ? '&scope=' + this.$route.query.scope : ''))
        .then((res) => {
    ...
    async excelExportReport ({ $axios }) {
      this.form.processing = true
      await this.$axios.$get(`/api/reports/locationsExcelExport?groups=${this.form.groups}&locations=${this.form.locations}` + (this.$route.query.scope ? '&scope=' + this.$route.query.scope : ''), { responseType: 'blob' })
        .then((response) => {
          this.form.processing = false
