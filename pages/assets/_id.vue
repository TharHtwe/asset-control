<template>
  <div class="content-wrapper">
    ...
    <div class="content">
      <div class="container-fluid">
        ...
        >
          ...
          <b-row>
            <!-- <b-col>
            <b-button v-if="allowCheckIn" variant="warning" :disabled="form.processing" @click.prevent="checkInAsset">
              Checkin
            </b-button>
            <b-button v-else variant="primary" :disabled="form.processing || !allowCheckOut" @click.prevent="checkOutAsset">
              Checkout
            </b-button>
          </b-col> -->
    ...
    <b-modal
      ...
      no-close-on-backdrop
      ok-title="Save"
      @ok="saveCheckOutAsset"
      @hidden="resetFormCheckOut"
    >
      <b-form>
        <b-form-group
          :label="float_label ? null : 'Date'"
        ...
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-datepicker
              v-model.trim="$v.form.checkout.checkout_date.$model"
              :disabled="form.processing"
              :state="
                form.submitted && !form.processing
                  ? !$v.form.checkout.checkout_date.$error
                  : null
              "
              placeholder=""
              name="recordDate"
        ...
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-select
              v-model="$v.form.checkout.checkout_type.$model"
              :enabled="!form.processing"
              :options="[
              ...
              :state="
                form.submitted && !form.processing
                  ? !$v.form.checkout.checkout_type.$error
                  : null
              "
            />
            <span v-if="float_label">Type</span>
        ...
        <b-form-group
          :label="float_label ? null : 'Checkout To'"
          :label-cols-sm="float_label ? null : 2"
        >
          <span :class="float_label ? 'has-float-label' : null">
            <v-select
              v-model="$v.form.checkout.checkout_to.$model"
              :options="mEmployees"
              label="name"
              taggable
              select-on-tab
              autocomplete="on"
              :reduce="(emp) => emp.name"
              :create-option="(emp) => ({ id: 0, name: emp })"
            />
            <span v-if="float_label">Checkout To</span>
          </span>
        </b-form-group>
        <b-form-group
          :label="float_label ? null : 'Quantity'"
        ...
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-input
              v-model.trim="$v.form.checkout.quantity.$model"
              type="number"
              step="1"
              min="1"
              :disabled="form.processing"
              :state="
                form.submitted && !form.processing
                  ? !$v.form.checkout.quantity.$error
                  : null
              "
            />
            <span v-if="float_label">Quantity</span>
        ...
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-textarea
              v-model.trim="form.checkout.remark"
              rows="2"
              max-rows="3"
              :disabled="form.processing"
            />
    ...
    <b-modal
      ...
      ok-title="Save"
      @ok="saveCheckInAsset"
      @hidden="resetFormCheckIn"
    >
      <b-form>
        <b-form-group
          :label="float_label ? null : 'Checkin From'"
          :label-cols-sm="float_label ? null : 2"
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-input
              v-model="form.checkin.checkin_from"
              :disabled="true"
            />
            <span v-if="float_label">Checkin From</span>
          </span>
        </b-form-group>
        <b-form-group
          :label="float_label ? null : 'Date'"
        ...
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-datepicker
              v-model.trim="$v.form.checkin.checkin_date.$model"
              :disabled="form.processing"
              :state="
                form.submitted && !form.processing
                  ? !$v.form.checkin.checkin_date.$error
                  : null
              "
              placeholder=""
              name="recordDate"
        ...
        >
          <span :class="float_label ? 'has-float-label' : null">
            <b-form-textarea
              v-model.trim="form.checkin.remark"
              rows="2"
              max-rows="3"
              :disabled="form.processing"
            />
...
export default {
  name: 'EditAsset',
  async asyncData({ $store, $axios, params, route, $snotify }) {
    let mAssetGroups = []
    let mEmployees = []
    let mAsset = null
    let mLocations = []
    await $axios
      .$get(`/api/assets/${params.id}/edit` + (route.query.scope ? '?scope=' + route.query.scope : ''))
      .then((res) => {
        mAsset = res.asset
        mAssetGroups = res.groups
        mEmployees = res.employees
        mLocations = res.locations
      })
      .catch((err) => {
        if(err.response) {
          $snotify.error(err.response.data.message)
        } else {
          $snotify.error(err.message)
        }
      })
    return { mAsset, mAssetGroups, mEmployees, mLocations }
  },
  data() {
    return {
  ...
  computed: {
    ...
    allowCheckOut() {
      if (
        this.form.asset.status != null &&
        this.form.asset.status.toLowerCase() === 'active'
      ) {
        return true
      } else {
        return false
    ...
    allowCheckIn() {
      if (['assigned', 'rented', 'used'].includes(this.form.asset.status)) {
        return true
      } else {
        return false
      }
  ...
  mounted() {
    if (this.mAsset !== null) {
      this.form.asset.id = this.mAsset.id
      this.form.asset.name = this.mAsset.name
      this.form.asset.group = this.mAsset.group
      this.form.asset.code = this.mAsset.code
      this.form.asset.serial_no = this.mAsset.serial_no
      this.form.asset.quantity = this.mAsset.quantity
      this.form.asset.details = this.mAsset.details
      this.form.asset.status = this.mAsset.status
      this.form.asset.photo = this.mAsset.photo
      this.form.asset.summarize_by_group = this.mAsset.summarize_by_group
      this.form.asset.summarize_by = this.mAsset.summarize_by
      this.form.records = this.mAsset.movements
    }
  },
  methods: {
    ...
    async saveCheckOutAsset(e) {
      e.preventDefault()
      this.form.submitted = true
      this.$v.form.checkout.$touch()
      if (this.$v.form.checkout.$anyError) {
        return
      }
      this.form.processing = true
      this.form.checkout.asset_id = this.form.asset.id
      await this.$axios
        .$post(`/api/assets/${this.form.asset.id}/checkout`, {
          data: JSON.stringify(this.form.checkout),
        })
        .then((res) => {
          this.form.asset.current_checkout = res.checkout
        })
        .then(() => {
    ...
    async saveCheckInAsset(e) {
      e.preventDefault()
      this.form.submitted = true
      this.$v.form.checkin.$touch()
      if (this.$v.form.checkin.$anyError) {
        return
      }
      this.form.processing = true
      this.form.checkin.asset_id = this.form.asset.id
      await this.$axios
        .$post(`/api/assets/${this.form.asset.id}/checkin`, {
          data: JSON.stringify(this.form.checkin),
        })
        .then(() => {
          this.$bvModal.hide('checkin-modal')
        })
        .catch(() => {})
    ...
    resetFormCheckOut() {
      this.form.checkout.id = 0
      this.form.checkout.asset_id = this.form.asset.id
      this.form.checkout.checkout_date = new Date()
      this.form.checkout.checkout_type = 'assigned'
      this.form.checkout.checkout_to = null
      this.form.checkout.quantity = 1
      this.form.checkout.remark = null
    },
    checkInAsset() {
    ...
    resetFormCheckIn() {
      this.form.checkin.id = this.form.asset.current_checkout.id
      this.form.checkin.asset_id = this.form.asset.id
      this.form.checkin.checkin_date = new Date()
      this.form.checkin.checkin_from =
        this.form.asset.current_checkout.checkout_to
      this.form.checkin.remark = this.form.asset.current_checkout.remark
    },
    initCropper() {
